...
export const FinancialRecordForm = () => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault(); //so it doesn't refresh when submit the form

    //Let's Statrt Creating the Structure of our new record : schema
    const newRecord = {
      userId: user?.id ?? "",
      date: new Date(),
    ...
    //<- This is gonna call our server and send our data : Post Req
    
    addRecord(newRecord);  
    
    setDescription(""); //<- after taking the data this will clear the input fields to empty
    setAmount("");
    setCategory("");
    setPaymentMethod("");  
};
